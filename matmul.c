...
{
#ifdef WITH_AVX2
    if (sizex * sizey > 256 * 256 && !((sizex & 15) || (sizey & 15) || (sizez & 15)))
    {
...
#pragma omp parallel for
    for (register size_t i = 0; i < sizex; i++)
    {
        for (register size_t j = 0; j < sizey; j++)
        {
            ...
            for (register size_t k = 0; k < sizez; k += 8)
            {
                c = _mm256_add_ps(c,
                    _mm256_mul_ps(_mm256_load_ps(A + ((i + luAx) * colA + k + luAy)),
                                  _mm256_load_ps(B + ((j + luBx) * colB + k + luBy))));
            }
...
#endif
    trans(b, C);
// The recursion method. Slightly slower.
//    blocking_matmul(a->data, a->n, 0, 0, C, b->m, 0, 0, ans->data, b->n, 0, 0, a->m, b->n, a->n);
// The loop method. Sometimes faster.
    size_t bsize = 256;
    if(a->n<bsize || b->m<bsize){
        return matmul_improved(a,b,ans);
    }
    for (register size_t kk = 0; kk < a->n; kk += bsize)
    {
        for (register size_t cc = 0; cc < b->n; cc += bsize)
        {
#pragma omp parallel for
            for (register size_t r = 0; r < a->m; r++)
            {
                for (register size_t c = cc; c < cc + bsize; c++)
                {
#ifdef WITH_AVX2
                    float sum[8];
                    __m256 ax,bx;
                    __m256 cc = _mm256_setzero_ps();
                    for (register size_t k = kk; k < kk + bsize; k+=8)
                    {
                        ax = _mm256_load_ps(a->data+(r * a->n + k));
                        bx = _mm256_load_ps(C+(c * b->m + k));
                        cc = _mm256_add_ps(cc, _mm256_mul_ps(ax, bx));
                    }
                    _mm256_store_ps(sum, cc);
                    ans->data[r*b->n + c] += sum[0] + sum[1] + sum[2] + sum[3] + sum[4] + sum[5] + sum[6] + sum[7];
#else
#ifdef WITH_NEON
                    float sum[4];
                    __m256 ax,bx;
                    __m256 cc = vdupq_n_f32(0);
                    for (register size_t k = kk; k < kk + bsize; k+=8)
                    {
                        ax = vld1q_f32(a->data+(r * a->n + k));
                        bx = vld1q_f32(C+(c * b->m + k));
                        cc = vaddq_f32(cc, vmulq_f32(ax, bx));
                    }
                    vst1q_f32(sum, cc);
                    ans->data[r*b->n + c] += sum[0] + sum[1] + sum[2] + sum[3];
#endif
#endif
                }
            }
...
Mat *newmat_aligned(const size_t m, const size_t n, const float *src)
{
    ...
    if (src)
        memcpy(ans->data, src, sz);
    else
        memset(ans->data, 0.0f, sz);
    return ans;
}
